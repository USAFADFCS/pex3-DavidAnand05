/** main.c
 * ===========================================================
 * Name: ______David ANand, __4 __23_ 2026
 * Section: CS483 / ____
...
int main(int argc, char **argv) {
    ...
    //void* calloc( size_t num, size_t size ); from online 
    ...
    while (!feof(ifp)) {
        ...
        */
        ...
        if (depth == -1) {
            for (int f = 1; f <= maxFrames; f++) faults[f]++;
        } else if (depth > 0) {
            for (long f = 1; f <= depth && f <= maxFrames; f++) faults[f]++;
        }   
